...
import { QG_GRID_HELM_IMPORTS } from "./qg-helm-utils";
import { buildPageTokens } from "./qg-pagination.util";

export type QgGridPaginationConfig = {
  currentPage: number;
  totalPages: number;
  totalItems: number;
  pageSize: number;
...
@Component({
  selector: "qg-grid-pagination",
  standalone: true,
  imports: [...QG_GRID_HELM_IMPORTS],
...
export class QgGridPaginationComponent {
  private readonly i18n = inject(QgI18nService);

  readonly config = input.required<QgGridPaginationConfig>();
