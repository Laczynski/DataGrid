...
import type { GridColumn, GridResource, GridSize, QgColumnContext } from "@query-grid/spartan";
import {
...
} from "../filter-editors/qg-column-filter.component";
...
import {
  QgGridPaginationComponent,
  type QgGridPaginationConfig,
} from "./qg-grid-pagination.component";
import {
...
import { type QgGridIconName, type QgGridMenuItem, resolveQgGridIcon } from "./qg-icon-map";
...
@Component({
  selector: "qg-hlm-query-grid",
  standalone: true,
  imports: GRID_IMPORTS,
  providers: [provideQgGridHelmIcons()],
  changeDetection: ChangeDetectionStrategy.OnPush,
  templateUrl: "./hlm-query-grid.component.html",
  styleUrl: "./hlm-query-grid.component.scss",
  host: {
    class: "qg-hlm-grid",
    "[class.qg-hlm-grid--small]": 'size() === "small"',
    "[class.qg-hlm-grid--medium]": 'size() === "medium"',
    "[class.qg-hlm-grid--large]": 'size() === "large"',
    "[class.qg-hlm-grid--striped]": "striped()",
    "[class.qg-hlm-grid--hoverable]": "hoverable()",
    "[class.qg-hlm-grid--bordered]": "hasGridlines()",
  },
})
export class HlmQueryGridComponent<T = unknown> {
    ...
    afterNextRender(
      () => {
        ...
        for (const header of Array.from(
          wrap.querySelectorAll<HTMLElement>(".qg-hlm-grid__header-cell[data-field]"),
        )) {
  ...
  protected readonly paginationConfig = computed<QgGridPaginationConfig>(() => {
  ...
  protected readonly exportAllMenuItems = computed<QgGridMenuItem[]>(() => {
    this.i18n.languageVersion()();
    return [
  ...
  protected readonly exportSelectedMenuItems = computed<QgGridMenuItem[]>(() => {
    this.i18n.languageVersion()();
    return [
      { id: "csv", label: this.i18n.t("grid.exportFormatCsv", "CSV (.csv)") },
      { id: "xlsx", label: this.i18n.t("grid.exportFormatXlsx", "Excel (.xlsx)") },
  ...
  protected onExportAllMenuItemClick(item: QgGridMenuItem): void {
    if (item.id === "csv") {
  ...
  protected onExportSelectedMenuItemClick(item: QgGridMenuItem): void {
    if (item.id === "csv") {
  ...
  protected pinIcon(column: GridColumn<T>): QgGridIconName {
  ...
  protected sortIcon(column: GridColumn<T>): QgGridIconName {
