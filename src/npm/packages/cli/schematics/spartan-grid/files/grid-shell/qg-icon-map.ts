...
export const QG_GRID_LUCIDE_ICONS = {
  lucideFilter,
  lucideListFilter,
  lucideFilterX,
  lucideColumns3,
...
const ICON_ALIASES: Record<string, keyof typeof QG_GRID_LUCIDE_ICONS> = {
  filter: "lucideFilter",
...
export function resolveQgGridIcon(name: string): keyof typeof QG_GRID_LUCIDE_ICONS {
  return ICON_ALIASES[name] ?? "lucideFilter";
}

export type QgGridMenuItem = {
  id: string;
  label: string;
  disabled?: boolean;
};

export type QgGridIconName = string;
