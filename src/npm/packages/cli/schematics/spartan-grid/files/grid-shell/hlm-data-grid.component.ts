...
import type { GridColumn, GridResource, GridSize, DgColumnContext } from "@laczynski/datagrid-spartan";
import {
...
} from "../filter-editors/dg-column-filter.component";
...
import {
  DgGridPaginationComponent,
  type DgGridPaginationConfig,
} from "./dg-grid-pagination.component";
import {
...
import { type DgGridIconName, type DgGridMenuItem, resolveDgGridIcon } from "./dg-icon-map";
...
@Component({
  selector: "dg-hlm-data-grid",
  standalone: true,
  imports: GRID_IMPORTS,
  providers: [provideDgGridHelmIcons()],
  changeDetection: ChangeDetectionStrategy.OnPush,
  templateUrl: "./hlm-data-grid.component.html",
  styleUrl: "./hlm-data-grid.component.scss",
  host: {
    class: "dg-hlm-grid",
    "[class.dg-hlm-grid--small]": 'size() === "small"',
    "[class.dg-hlm-grid--medium]": 'size() === "medium"',
    "[class.dg-hlm-grid--large]": 'size() === "large"',
    "[class.dg-hlm-grid--striped]": "striped()",
    "[class.dg-hlm-grid--hoverable]": "hoverable()",
    "[class.dg-hlm-grid--bordered]": "hasGridlines()",
  },
})
export class HlmDataGridComponent<T = unknown> {
    ...
    afterNextRender(
      () => {
        ...
        for (const header of Array.from(
          wrap.querySelectorAll<HTMLElement>(".dg-hlm-grid__header-cell[data-field]"),
        )) {
  ...
  protected readonly paginationConfig = computed<DgGridPaginationConfig>(() => {
  ...
  protected readonly exportAllMenuItems = computed<DgGridMenuItem[]>(() => {
    this.i18n.languageVersion()();
    return [
      { id: "csv", label: this.i18n.t("grid.exportFormatCsv", "CSV (.csv)") },
      { id: "xlsx", label: this.i18n.t("grid.exportFormatXlsx", "Excel (.xlsx)") },
    ];
  });

  protected readonly exportSelectedMenuItems = computed<DgGridMenuItem[]>(() => {
    this.i18n.languageVersion()();
    return [
      { id: "csv", label: this.i18n.t("grid.exportFormatCsv", "CSV (.csv)") },
      { id: "xlsx", label: this.i18n.t("grid.exportFormatXlsx", "Excel (.xlsx)") },
  ...
  protected onExportAllMenuItemClick(item: DgGridMenuItem): void {
    if (item.id === "csv") {
  ...
  protected onExportSelectedMenuItemClick(item: DgGridMenuItem): void {
    if (item.id === "csv") {
  ...
  protected pinnedZIndex(field: string, section: "header" | "body"): number | undefined {
    ...
    const base = section === "header" ? 20 : 5;
    let pinIndex = 0;
  ...
  protected pinIcon(column: GridColumn<T>): DgGridIconName {
  ...
  protected sortIcon(column: GridColumn<T>): DgGridIconName {
